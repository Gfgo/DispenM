EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 1
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L Motor:Motor_Servo M1
U 1 1 6484B47B
P 6650 2450
F 0 "M1" H 6550 2450 50  0000 L CNN
F 1 "Motor_Servo" H 6500 2350 50  0000 L CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x03_P2.54mm_Vertical" H 6650 2260 50  0001 C CNN
F 3 "http://forums.parallax.com/uploads/attachments/46831/74481.png" H 6650 2260 50  0001 C CNN
	1    6650 2450
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x03 NiHALL1
U 1 1 6484BAFB
P 3850 2850
F 0 "NiHALL1" H 3768 3167 50  0000 C CNN
F 1 "Conn_01x03" H 3768 3076 50  0000 C CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x03_P2.54mm_Vertical" H 3850 2850 50  0001 C CNN
F 3 "~" H 3850 2850 50  0001 C CNN
	1    3850 2850
	-1   0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x08 OLED1
U 1 1 6484BF16
P 3150 3650
F 0 "OLED1" H 3068 4167 50  0000 C CNN
F 1 "Conn_01x08" H 3068 4076 50  0000 C CNN
F 2 "MODULOS:OLED128X128" H 3150 3650 50  0001 C CNN
F 3 "~" H 3150 3650 50  0001 C CNN
	1    3150 3650
	-1   0    0    -1  
$EndComp
$Comp
L Timer_RTC:DS3231MZ U2
U 1 1 6484C580
P 6600 1500
F 0 "U2" H 6600 1550 50  0000 C CNN
F 1 "DS3231MZ" H 6400 1450 50  0000 C CNN
F 2 "MODULOS:DS3231MH" H 6600 1000 50  0001 C CNN
F 3 "http://datasheets.maximintegrated.com/en/ds/DS3231M.pdf" H 6600 900 50  0001 C CNN
	1    6600 1500
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x03 BOTO2
U 1 1 6484CD6F
P 6750 4100
F 0 "BOTO2" H 6668 4417 50  0000 C CNN
F 1 "Conn_01x03" H 6668 4326 50  0000 C CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x03_P2.54mm_Vertical" H 6750 4100 50  0001 C CNN
F 3 "~" H 6750 4100 50  0001 C CNN
	1    6750 4100
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x03 BOTO1
U 1 1 6484ED92
P 6750 3350
F 0 "BOTO1" H 6668 3667 50  0000 C CNN
F 1 "Conn_01x03" H 6668 3576 50  0000 C CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x03_P2.54mm_Vertical" H 6750 3350 50  0001 C CNN
F 3 "~" H 6750 3350 50  0001 C CNN
	1    6750 3350
	1    0    0    -1  
$EndComp
$Comp
L power:+3.3V #PWR0101
U 1 1 64852F69
P 3350 3350
F 0 "#PWR0101" H 3350 3200 50  0001 C CNN
F 1 "+3.3V" V 3365 3478 50  0000 L CNN
F 2 "" H 3350 3350 50  0001 C CNN
F 3 "" H 3350 3350 50  0001 C CNN
	1    3350 3350
	0    1    1    0   
$EndComp
$Comp
L power:+3.3V #PWR0102
U 1 1 648545ED
P 6550 3350
F 0 "#PWR0102" H 6550 3200 50  0001 C CNN
F 1 "+3.3V" V 6565 3478 50  0000 L CNN
F 2 "" H 6550 3350 50  0001 C CNN
F 3 "" H 6550 3350 50  0001 C CNN
	1    6550 3350
	0    -1   -1   0   
$EndComp
$Comp
L power:+3.3V #PWR0103
U 1 1 64855A2C
P 6550 4100
F 0 "#PWR0103" H 6550 3950 50  0001 C CNN
F 1 "+3.3V" V 6565 4228 50  0000 L CNN
F 2 "" H 6550 4100 50  0001 C CNN
F 3 "" H 6550 4100 50  0001 C CNN
	1    6550 4100
	0    -1   -1   0   
$EndComp
$Comp
L power:+5V #PWR0104
U 1 1 64856200
P 6350 2450
F 0 "#PWR0104" H 6350 2300 50  0001 C CNN
F 1 "+5V" V 6365 2578 50  0000 L CNN
F 2 "" H 6350 2450 50  0001 C CNN
F 3 "" H 6350 2450 50  0001 C CNN
	1    6350 2450
	0    -1   -1   0   
$EndComp
$Comp
L power:+5V #PWR0105
U 1 1 64856E0D
P 6600 1100
F 0 "#PWR0105" H 6600 950 50  0001 C CNN
F 1 "+5V" H 6615 1273 50  0000 C CNN
F 2 "" H 6600 1100 50  0001 C CNN
F 3 "" H 6600 1100 50  0001 C CNN
	1    6600 1100
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0106
U 1 1 6485769F
P 3850 3350
F 0 "#PWR0106" H 3850 3200 50  0001 C CNN
F 1 "+5V" V 3865 3478 50  0000 L CNN
F 2 "" H 3850 3350 50  0001 C CNN
F 3 "" H 3850 3350 50  0001 C CNN
	1    3850 3350
	0    1    1    0   
$EndComp
NoConn ~ 7100 1300
NoConn ~ 7100 1600
NoConn ~ 6100 1700
$Comp
L power:GND #PWR0107
U 1 1 6485A77A
P 4050 2950
F 0 "#PWR0107" H 4050 2700 50  0001 C CNN
F 1 "GND" V 4055 2822 50  0000 R CNN
F 2 "" H 4050 2950 50  0001 C CNN
F 3 "" H 4050 2950 50  0001 C CNN
	1    4050 2950
	0    -1   -1   0   
$EndComp
$Comp
L power:GND #PWR0108
U 1 1 6485B27F
P 3850 3650
F 0 "#PWR0108" H 3850 3400 50  0001 C CNN
F 1 "GND" V 3855 3522 50  0000 R CNN
F 2 "" H 3850 3650 50  0001 C CNN
F 3 "" H 3850 3650 50  0001 C CNN
	1    3850 3650
	0    -1   -1   0   
$EndComp
$Comp
L power:GND #PWR0109
U 1 1 6485B66E
P 6600 1900
F 0 "#PWR0109" H 6600 1650 50  0001 C CNN
F 1 "GND" H 6605 1727 50  0000 C CNN
F 2 "" H 6600 1900 50  0001 C CNN
F 3 "" H 6600 1900 50  0001 C CNN
	1    6600 1900
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0110
U 1 1 6485BFB9
P 6550 3450
F 0 "#PWR0110" H 6550 3200 50  0001 C CNN
F 1 "GND" V 6555 3322 50  0000 R CNN
F 2 "" H 6550 3450 50  0001 C CNN
F 3 "" H 6550 3450 50  0001 C CNN
	1    6550 3450
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR0111
U 1 1 6485CA16
P 6550 4200
F 0 "#PWR0111" H 6550 3950 50  0001 C CNN
F 1 "GND" V 6555 4072 50  0000 R CNN
F 2 "" H 6550 4200 50  0001 C CNN
F 3 "" H 6550 4200 50  0001 C CNN
	1    6550 4200
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR0112
U 1 1 6485CF38
P 6350 2550
F 0 "#PWR0112" H 6350 2300 50  0001 C CNN
F 1 "GND" V 6355 2422 50  0000 R CNN
F 2 "" H 6350 2550 50  0001 C CNN
F 3 "" H 6350 2550 50  0001 C CNN
	1    6350 2550
	0    1    1    0   
$EndComp
$Comp
L power:+5V #PWR0113
U 1 1 6485D4A5
P 4050 2850
F 0 "#PWR0113" H 4050 2700 50  0001 C CNN
F 1 "+5V" V 4065 2978 50  0000 L CNN
F 2 "" H 4050 2850 50  0001 C CNN
F 3 "" H 4050 2850 50  0001 C CNN
	1    4050 2850
	0    1    1    0   
$EndComp
Wire Wire Line
	6200 4000 6200 3550
Wire Wire Line
	6200 3550 5800 3550
Text GLabel 5800 3650 2    50   Input ~ 0
RST
Text GLabel 5800 3750 2    50   Input ~ 0
SCL
Text GLabel 5800 3850 2    50   Input ~ 0
SDA
Text GLabel 5800 3350 2    50   Input ~ 0
SERVO
Text GLabel 6100 1300 0    50   Input ~ 0
SCL
Text GLabel 6100 1400 0    50   Input ~ 0
SDA
Text GLabel 6350 2350 0    50   Input ~ 0
SERVO
$Comp
L power:GND #PWR0114
U 1 1 64865064
P 4600 2650
F 0 "#PWR0114" H 4600 2400 50  0001 C CNN
F 1 "GND" V 4605 2522 50  0000 R CNN
F 2 "" H 4600 2650 50  0001 C CNN
F 3 "" H 4600 2650 50  0001 C CNN
	1    4600 2650
	0    1    1    0   
$EndComp
Text GLabel 4600 3150 0    50   Input ~ 0
DC
Text GLabel 4600 3250 0    50   Input ~ 0
CS
Text GLabel 4600 3350 0    50   Input ~ 0
SCK
Text GLabel 4600 3450 0    50   Input ~ 0
SI
Wire Wire Line
	4600 2750 4050 2750
Text GLabel 3350 3650 2    50   Input ~ 0
RST
Text GLabel 3350 3750 2    50   Input ~ 0
SCK
Text GLabel 3350 3850 2    50   Input ~ 0
SI
Text GLabel 3350 3950 2    50   Input ~ 0
CS
Text GLabel 3350 4050 2    50   Input ~ 0
DC
Wire Wire Line
	6200 4000 6550 4000
NoConn ~ 5800 3450
NoConn ~ 5800 3150
NoConn ~ 5800 3050
NoConn ~ 5800 2850
NoConn ~ 5800 2750
NoConn ~ 4600 2350
NoConn ~ 4600 2550
NoConn ~ 4600 3550
NoConn ~ 4600 3650
NoConn ~ 4600 3750
NoConn ~ 4600 3850
NoConn ~ 6500 1100
$Comp
L Diode:1N4001 D1
U 1 1 64B84623
P 3850 3500
F 0 "D1" V 3804 3580 50  0000 L CNN
F 1 "1N4001" V 3895 3580 50  0000 L CNN
F 2 "Diode_THT:D_DO-41_SOD81_P3.81mm_Vertical_KathodeUp" H 3850 3325 50  0001 C CNN
F 3 "http://www.vishay.com/docs/88503/1n4001.pdf" H 3850 3500 50  0001 C CNN
	1    3850 3500
	0    1    1    0   
$EndComp
Wire Wire Line
	3850 3350 3750 3350
Wire Wire Line
	3750 3350 3750 3450
Wire Wire Line
	3750 3450 3350 3450
Connection ~ 3850 3350
Wire Wire Line
	3350 3550 3750 3550
Wire Wire Line
	3750 3550 3750 3650
Wire Wire Line
	3750 3650 3850 3650
Connection ~ 3850 3650
$Comp
L power:+3.3V #PWR0117
U 1 1 64B94797
P 4600 2450
F 0 "#PWR0117" H 4600 2300 50  0001 C CNN
F 1 "+3.3V" V 4615 2578 50  0000 L CNN
F 2 "" H 4600 2450 50  0001 C CNN
F 3 "" H 4600 2450 50  0001 C CNN
	1    4600 2450
	0    -1   -1   0   
$EndComp
NoConn ~ 4600 2850
NoConn ~ 4600 2950
NoConn ~ 4600 3050
Wire Wire Line
	6550 3250 5800 3250
$Comp
L LoRa32U4:LoRa32U4 U1
U 1 1 64BFF81C
P 5200 2750
F 0 "U1" H 5200 3515 50  0000 C CNN
F 1 "LoRa32U4" H 5200 3424 50  0000 C CNN
F 2 "MODULOS:Lora32U4" H 5200 2750 50  0001 C CNN
F 3 "" H 5200 2750 50  0001 C CNN
	1    5200 2750
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0118
U 1 1 64C03C45
P 5800 2950
F 0 "#PWR0118" H 5800 2800 50  0001 C CNN
F 1 "+5V" V 5815 3078 50  0000 L CNN
F 2 "" H 5800 2950 50  0001 C CNN
F 3 "" H 5800 2950 50  0001 C CNN
	1    5800 2950
	0    1    1    0   
$EndComp
$Comp
L Regulator_Linear:LM7805_TO220 U4
U 1 1 651AE70B
P 4450 1350
F 0 "U4" H 4450 1592 50  0000 C CNN
F 1 "LM7805_TO220" H 4450 1501 50  0000 C CNN
F 2 "Package_TO_SOT_THT:TO-220-3_Vertical" H 4450 1575 50  0001 C CIN
F 3 "https://www.onsemi.cn/PowerSolutions/document/MC7800-D.PDF" H 4450 1300 50  0001 C CNN
	1    4450 1350
	-1   0    0    -1  
$EndComp
$Comp
L Regulator_Linear:LD1117S33TR_SOT223 U3
U 1 1 651AF6C9
P 3050 1350
F 0 "U3" H 3050 1592 50  0000 C CNN
F 1 "LD1117S33TR_SOT223" H 3050 1501 50  0000 C CNN
F 2 "Package_TO_SOT_THT:TO-220-3_Vertical" H 3050 1550 50  0001 C CNN
F 3 "http://www.st.com/st-web-ui/static/active/en/resource/technical/document/datasheet/CD00000544.pdf" H 3150 1100 50  0001 C CNN
	1    3050 1350
	-1   0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x02 J1
U 1 1 651B1155
P 5100 1350
F 0 "J1" H 5180 1342 50  0000 L CNN
F 1 "Conn_01x02" H 5180 1251 50  0000 L CNN
F 2 "TerminalBlock_Phoenix:TerminalBlock_Phoenix_MKDS-1,5-2_1x02_P5.00mm_Horizontal" H 5100 1350 50  0001 C CNN
F 3 "~" H 5100 1350 50  0001 C CNN
	1    5100 1350
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0116
U 1 1 651B75A5
P 3950 1650
F 0 "#PWR0116" H 3950 1400 50  0001 C CNN
F 1 "GND" H 3955 1477 50  0000 C CNN
F 2 "" H 3950 1650 50  0001 C CNN
F 3 "" H 3950 1650 50  0001 C CNN
	1    3950 1650
	1    0    0    -1  
$EndComp
Connection ~ 3950 1650
$Comp
L power:+3.3V #PWR0119
U 1 1 651B78BF
P 2750 1350
F 0 "#PWR0119" H 2750 1200 50  0001 C CNN
F 1 "+3.3V" V 2765 1478 50  0000 L CNN
F 2 "" H 2750 1350 50  0001 C CNN
F 3 "" H 2750 1350 50  0001 C CNN
	1    2750 1350
	0    -1   -1   0   
$EndComp
Wire Wire Line
	3950 1650 4450 1650
Wire Wire Line
	3050 1650 3950 1650
Wire Wire Line
	4450 1650 4900 1650
Wire Wire Line
	4900 1650 4900 1450
Connection ~ 4450 1650
Wire Wire Line
	4750 1350 4900 1350
Wire Wire Line
	4150 1350 3750 1350
$Comp
L power:+5V #PWR0115
U 1 1 65257AC4
P 3750 1350
F 0 "#PWR0115" H 3750 1200 50  0001 C CNN
F 1 "+5V" H 3765 1523 50  0000 C CNN
F 2 "" H 3750 1350 50  0001 C CNN
F 3 "" H 3750 1350 50  0001 C CNN
	1    3750 1350
	1    0    0    -1  
$EndComp
Connection ~ 3750 1350
Wire Wire Line
	3750 1350 3350 1350
$Comp
L Device:R CABLE1
U 1 1 654A6BE8
P 5000 4300
F 0 "CABLE1" V 4793 4300 50  0000 C CNN
F 1 "R" V 4884 4300 50  0000 C CNN
F 2 "Resistor_THT:R_Axial_DIN0411_L9.9mm_D3.6mm_P20.32mm_Horizontal" V 4930 4300 50  0001 C CNN
F 3 "~" H 5000 4300 50  0001 C CNN
	1    5000 4300
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR0120
U 1 1 654A7900
P 4850 4300
F 0 "#PWR0120" H 4850 4050 50  0001 C CNN
F 1 "GND" V 4855 4172 50  0000 R CNN
F 2 "" H 4850 4300 50  0001 C CNN
F 3 "" H 4850 4300 50  0001 C CNN
	1    4850 4300
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR0121
U 1 1 654A812A
P 5150 4300
F 0 "#PWR0121" H 5150 4050 50  0001 C CNN
F 1 "GND" H 5155 4127 50  0000 C CNN
F 2 "" H 5150 4300 50  0001 C CNN
F 3 "" H 5150 4300 50  0001 C CNN
	1    5150 4300
	0    -1   -1   0   
$EndComp
$Comp
L Device:R CABLE2
U 1 1 654B1BB8
P 5000 4650
F 0 "CABLE2" V 4793 4650 50  0000 C CNN
F 1 "R" V 4884 4650 50  0000 C CNN
F 2 "Resistor_THT:R_Axial_DIN0411_L9.9mm_D3.6mm_P20.32mm_Horizontal" V 4930 4650 50  0001 C CNN
F 3 "~" H 5000 4650 50  0001 C CNN
	1    5000 4650
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR0122
U 1 1 654B214E
P 4850 4650
F 0 "#PWR0122" H 4850 4400 50  0001 C CNN
F 1 "GND" V 4855 4522 50  0000 R CNN
F 2 "" H 4850 4650 50  0001 C CNN
F 3 "" H 4850 4650 50  0001 C CNN
	1    4850 4650
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR0123
U 1 1 654B2579
P 5150 4650
F 0 "#PWR0123" H 5150 4400 50  0001 C CNN
F 1 "GND" H 5155 4477 50  0000 C CNN
F 2 "" H 5150 4650 50  0001 C CNN
F 3 "" H 5150 4650 50  0001 C CNN
	1    5150 4650
	0    -1   -1   0   
$EndComp
$EndSCHEMATC
